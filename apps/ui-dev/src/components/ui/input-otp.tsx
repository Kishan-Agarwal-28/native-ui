...
const useOTP = (): OTPContextValue => {
  ...
  if (!ctx) {
    throw new Error(
      "InputOTPSlot / InputOTPGroup must be rendered inside <InputOTP>.",
    );
  }
  return ctx;
};
...
const InputOTP = forwardRef<TextInput, InputOTPProps>(
  (
    ...
    return (
      <OTPContext.Provider
        value={{ value, maxLength, isFocused, activeIndex, focusInput }}
      >
        <Pressable
          onPress={focusInput}
          disabled={disabled}
          ...
          accessibilityLabel={
            accessibilityLabel ??
            `One-time password, ${value.length} of ${maxLength} digits entered`
          }
...
const InputOTPSlot = forwardRef<View, InputOTPSlotProps>(
  ({ index, style, charStyle, ...rest }, ref) => {
    const { theme } = useTheme();
      ...
      if (showCursor) {
        cursorOpacity.value = withRepeat(
          withSequence(
            ...
            withTiming(0, { duration: BLINK_FADE_MS }),
            withTiming(0, { duration: BLINK_OFF_MS }),
          ),
          -1,
        );
      } else {
    ...
    return (
      ...
      >
        {char !== null ? (
          <Animated.View key="char" style={animatedChar}>
            <Text
              style={[
                {
