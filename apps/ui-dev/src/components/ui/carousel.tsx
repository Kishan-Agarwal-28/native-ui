...
const Carousel = forwardRef<View, CarouselProps>(
  (
    ...
    const scrollToVirtualIndex = useCallback(
      (index: number) => {
        if (itemWidth === 0 || totalItems === 0) return;
        const target = loop
          ? index
    ...
    useEffect(() => {
      if (autoplayInterval <= 0 || totalItems <= 1) return;
...
type CarouselContentProps = Omit<
  ...
  "data" | "renderItem" | "horizontal" | "ref"
> & {
  style?: StyleProp<ViewStyle>;
};
...
>(({ children, style, ...props }, forwardedRef) => {
  ...
  const data = useMemo(() => {
    return isLooping
      ? Array(LOOPS_COUNT).fill(originalData).flat()
      : originalData;
  ...
  const setRef = useCallback(
    (node: FlatList<React.ReactNode> | null) => {
      (
        flatListRef as React.MutableRefObject<FlatList<React.ReactNode> | null>
      ).current = node;
      if (typeof forwardedRef === "function") forwardedRef(node);
      else if (forwardedRef) {
        (
          forwardedRef as React.MutableRefObject<FlatList<React.ReactNode> | null>
        ).current = node;
      }
    },
  ...
  const onViewableItemsChanged = useCallback(
    ({ viewableItems }: { viewableItems: ViewToken[] }) => {
      ...
      if (index != null) setVirtualIndex(index);
    },
    [setVirtualIndex],
  );
  ...
  return (
    <View
      style={[styles.flatListWrapper, style]}
      onLayout={(e) => {
...
const CarouselItem = forwardRef<View, CarouselItemProps>(
  ({ children, style, ...props }, ref) => {
    ...
    return (
      <View ref={ref} style={[sizeStyle, style]} {...props}>
        {children}
      </View>
    );
  },
...
function NavButton({ side, onPress, disabled, style }: NavButtonProps) {
  ...
  return (
    <View
      ...
      style={[styles.navWrapperBase, wrapperStyle]}
    >
      <Pressable
        onPress={onPress}
        disabled={disabled}
      ...
      >
        {isPrev ? (
          <ChevronLeft color={colors.foreground} size={16} strokeWidth={2} />
        ) : (
          <ChevronRight color={colors.foreground} size={16} strokeWidth={2} />
        )}
      </Pressable>
    </View>
  );
...
const CarouselPrevious = forwardRef<View, CarouselNavProps>(
  ({ style }, _ref) => {
...
const CarouselNext = forwardRef<View, CarouselNavProps>(({ style }, _ref) => {
  const { virtualIndex, totalItems, scrollToVirtualIndex, loop } =
    useCarouselContext();
...
const CarouselDots = forwardRef<View, CarouselNavProps>(({ style }, ref) => {
  ...
  const { virtualIndex, totalItems, scrollToVirtualIndex, orientation } =
    useCarouselContext();
